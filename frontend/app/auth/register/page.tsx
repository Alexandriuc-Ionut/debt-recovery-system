  ...
  return (
    <div className="mt-2.5 space-y-2">
      <div className="flex gap-1">
        {[1,2,3,4,5].map((i) => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= score ? barColor : "bg-slate-200"}`} />
        ))}
      </div>
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap gap-x-3 gap-y-1">
          {checks.map(({ label: l, ok }) => (
            <span key={l} className={`text-xs flex items-center gap-1 ${ok ? "text-emerald-600" : "text-slate-400"}`}>
              {ok ? <Check className="w-3 h-3" strokeWidth={3} /> : <span className="w-3 h-3 inline-flex items-center justify-center">·</span>}
              {l}
            </span>
          ))}
        </div>
        <span className={`text-xs font-semibold flex-shrink-0 ml-2 ${score >= 4 ? "text-emerald-600" : "text-slate-500"}`}>{label}</span>
      </div>
    </div>
  );
}
...
export default function RegisterPage() {
  ...
  const inputBase = (field: string) =>
    `w-full px-4 py-3 rounded-xl border text-sm bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all ${
      errors[field] ? "border-red-500/60 bg-red-50" : "border-slate-200 hover:border-slate-300"
    }`;
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      ...
      {/* Left panel */}
      <div className="hidden lg:flex lg:w-5/12 flex-col justify-between p-14 relative overflow-hidden border-r border-slate-200 bg-white">
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-2 mb-10">
            ...
            <p className="text-slate-600 text-sm leading-relaxed">{t.auth.panelSubtitle}</p>
          </div>
            ...
            {[
              ...
              <div
                ...
                className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                  step === num ? "bg-blue-50 border-blue-200" :
                  step > num ? "bg-emerald-50 border-emerald-200" :
                  "bg-slate-50 border-slate-200"
                }`}
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  step > num ? "bg-emerald-100 border border-emerald-300" :
                  step === num ? "bg-blue-100 border border-blue-300" :
                  "bg-slate-100 border border-slate-300"
                }`}>
                  {step > num
                    ? <Check className="w-4 h-4 text-emerald-600" strokeWidth={2.5} />
                    : <Icon className={`w-4 h-4 ${step === num ? "text-blue-600" : "text-slate-400"}`} />}
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-medium">{num === 1 ? t.auth.step1of2 : t.auth.step2of2}</p>
                  <p className={`text-sm font-semibold ${step === num ? "text-slate-900" : step > num ? "text-emerald-600" : "text-slate-500"}`}>{label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-start justify-center px-6 py-12 overflow-y-auto">
        <div className="w-full max-w-lg">
          ...
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            {[1, 2].map((n) => (
              <div key={n} className={`h-1 flex-1 rounded-full transition-all ${n <= step ? "bg-blue-500" : "bg-slate-200"}`} />
            ))}
          </div>
          ...
          {step === 1 ? (
            <>
              <div className="mb-8">
                ...
                <h2 className="text-2xl font-bold text-slate-900 tracking-tight">{t.auth.accountDetails}</h2>
                <p className="text-slate-500 text-sm mt-1">{t.auth.accountDetailsSubtitle}</p>
              </div>

              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.fullName} <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.emailAddress} <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.phone} <span className="text-slate-400 text-xs font-normal">({t.auth.optional})</span></label>
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+40 700 000 000" className={inputBase("phone")} />
                </div>

                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.password} <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.confirmPassword} <span className="text-red-500">*</span></label>
                ...
                <button type="button" onClick={() => { if (validateStep1()) setStep(2); }} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 rounded-xl text-sm transition-all shadow-lg shadow-blue-500/20 mt-2">
            ...
            <>
              <div className="mb-8">
                ...
                <h2 className="text-2xl font-bold text-slate-900 tracking-tight">{t.auth.companyInfo}</h2>
                <p className="text-slate-500 text-sm mt-1">{t.auth.companyInfoSubtitle}</p>
              </div>

              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.cui} <span className="text-slate-400 text-xs font-normal">({t.auth.cuiHelper})</span></label>
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={lookupAnaf} disabled={anafLoading} className="flex items-center gap-1.5 px-4 py-3 bg-slate-100 hover:bg-slate-200 border border-slate-200 hover:border-slate-300 disabled:opacity-40 text-slate-700 text-sm font-medium rounded-xl transition-all flex-shrink-0">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.companyName} <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.address}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.city}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.county}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.companyPhone}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">{t.auth.companyEmail}</label>
                ...
                <div className="flex gap-3 mt-2">
                  <button type="button" onClick={() => setStep(1)} className="flex items-center gap-1.5 px-5 py-3 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 font-medium rounded-xl text-sm transition-all">
                    <ChevronLeft className="w-4 h-4" /> {t.auth.back}
                  </button>
                  <button type="button" onClick={handleSubmit} disabled={loading} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-300 disabled:text-blue-100 text-white font-semibold py-3 rounded-xl text-sm transition-all shadow-lg shadow-blue-500/20">
          ...
          <p className="text-center text-sm text-slate-500 mt-8">
            {t.auth.hasAccount}{" "}
            <Link href="/auth/login" className="text-blue-600 hover:text-blue-700 font-semibold transition-colors">{t.auth.signIn}</Link>
          </p>
        </div>
      </div>
    </div>
