...
import {
  ...
  Users, Menu, X, Star, ArrowUpRight, Lock, Globe,
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between gap-6">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm text-slate-400 font-medium">
          {navLinks.map(({ label, href }) => (
            <a key={href} href={href} onClick={(e) => handleNavClick(e, href)} className="hover:text-white transition-colors">{label}</a>
          ))}
        </div>
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto">
      ...
      <div className="relative rounded-2xl border border-white/[0.1] bg-[#0d1829]/90 backdrop-blur-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="rounded-xl bg-white/[0.02] border border-white/[0.05] overflow-hidden">
            ...
            {[
              { client: 'Modatex Fashion S.A.', inv: 'INV-2024-041', amount: '€24,500', status: 'PAID', color: 'text-emerald-400 bg-emerald-500/10' },
              { client: 'Aura Wear Group S.R.L.', inv: 'INV-2024-042', amount: '€18,200', status: 'OPEN', color: 'text-amber-400 bg-amber-500/10' },
              { client: 'Romtex Distribution S.A.', inv: 'INV-2024-043', amount: '€9,800', status: 'OVERDUE', color: 'text-red-400 bg-red-500/10' },
            ].map(({ client, inv, amount, status, color }) => (
              <div key={inv} className="flex items-center justify-between px-3 py-2 border-b border-white/[0.03] last:border-0">
      ...
      <div className="absolute -right-6 top-24 w-44 rounded-xl bg-[#0d1829] border border-white/[0.1] p-3 shadow-2xl hidden lg:block">
        ...
        <p className="text-[9px] text-slate-500 mb-2">Aura Wear · Updated now</p>
      ...
      <div className="absolute -left-6 bottom-20 w-48 rounded-xl bg-[#0d1829] border border-white/[0.1] p-3 shadow-2xl hidden lg:block">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-500">Trend Wear S.R.L. · Due in 3d</p>
          </div>
        </div>
      </div>
    </div>
...
const TRUSTED_COMPANIES = [
  { name: 'Modatex Fashion S.A.', initial: 'M', industry: 'Textile manufacturing' },
  { name: 'Aura Wear Group S.R.L.', initial: 'A', industry: 'Clothing wholesale' },
  { name: 'Romtex Distribution S.A.', initial: 'R', industry: 'Fabric distribution' },
  { name: 'Trend Wear S.R.L.', initial: 'T', industry: 'Fashion retail' },
  { name: 'Elegance Impex S.A.', initial: 'E', industry: 'Import & export' },
  { name: 'StyleCorp Romania S.R.L.', initial: 'S', industry: 'Apparel supply chain' },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080d14] text-white overflow-x-hidden">
      ...
      <section className="py-16 px-5 sm:px-8 border-y border-white/[0.05]">
        <div className="max-w-5xl mx-auto">
          <p className="text-center text-xs font-bold text-slate-600 uppercase tracking-widest mb-10">
            Trusted by businesses across Romania
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {TRUSTED_COMPANIES.map(({ name, initial, industry }) => (
              <div key={name} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-colors">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600/80 to-indigo-700/80 flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-blue-900/30">
                  {initial}
                </div>
                <p className="text-[10px] font-semibold text-slate-300 text-center leading-tight">{name}</p>
                <p className="text-[9px] text-slate-600 text-center">{industry}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── STATS ──────────────────────────────────────────────────────────── */}
      <section className="py-16 px-5 sm:px-8 border-b border-white/[0.05]">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <div className="space-y-0">
                <Step n={1} icon={Users} title={t.landing.howItWorks.step1Title} desc={t.landing.howItWorks.step1Desc} />
                <Step n={2} icon={FileText} title={t.landing.howItWorks.step2Title} desc={t.landing.howItWorks.step2Desc} />
                <Step n={3} icon={Brain} title={t.landing.howItWorks.step3Title} desc={t.landing.howItWorks.step3Desc} />
                <Step n={4} icon={Bell} title={t.landing.howItWorks.step4Title} desc={t.landing.howItWorks.step4Desc} />
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl bg-white/[0.03] border border-white/[0.06] overflow-hidden">
                <div className="p-6 border-b border-white/[0.06]">
                  ...
                  {[
                    { name: 'Modatex Fashion S.A.', score: 92, risk: 'LOW', color: 'bg-emerald-500', textColor: 'text-emerald-400', badge: 'text-emerald-400 bg-emerald-500/10' },
                    { name: 'Aura Wear Group S.R.L.', score: 78, risk: 'LOW', color: 'bg-emerald-500', textColor: 'text-emerald-400', badge: 'text-emerald-400 bg-emerald-500/10' },
                    { name: 'Romtex Distribution S.A.', score: 55, risk: 'MEDIUM', color: 'bg-amber-400', textColor: 'text-amber-400', badge: 'text-amber-400 bg-amber-500/10' },
                    { name: 'Elegance Impex S.A.', score: 22, risk: 'HIGH', color: 'bg-red-500', textColor: 'text-red-400', badge: 'text-red-400 bg-red-500/10' },
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3">
                    {[
                      { icon: Bell, color: 'bg-amber-500/20 text-amber-400', text: 'Reminder sent to Elegance Impex S.A.', time: '2 min ago' },
                      { icon: CheckCircle2, color: 'bg-emerald-500/20 text-emerald-400', text: 'Payment received — Modatex Fashion S.A.', time: '1 hour ago' },
                      { icon: FileText, color: 'bg-blue-500/20 text-blue-400', text: 'Invoice INV-2024-041 generated', time: '3 hours ago' },
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              { icon: Lock, gradient: 'from-slate-500 to-slate-700', glow: 'shadow-slate-500/20',
              ...
              { icon: Globe, gradient: 'from-cyan-500 to-blue-600', glow: 'shadow-cyan-500/20',
                title: 'ANAF & e-Factura integrated',
                desc: 'Auto-populate company data via CUI lookup. Submit invoices to ANAF SPV in UBL 2.1 XML format as required by Romanian law since January 2024.' },
      ...
      <section className="py-20 px-5 sm:px-8 border-t border-white/[0.05]">
        <div className="max-w-3xl mx-auto text-center relative">
          ...
          <div className="relative rounded-3xl bg-gradient-to-br from-blue-600/20 to-indigo-600/10 border border-blue-500/20 p-12 shadow-2xl">
            ...
            <p className="text-slate-400 text-lg mb-8 leading-relaxed">{t.landing.cta.subtitle}</p>
